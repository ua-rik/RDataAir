...
jobdata %>%
  dplyr::group_by(station_ID, coodrinates, date, time, indicator) %>%
  dplyr::summarise(n = dplyr::n(), .groups = "drop") %>%
  dplyr::filter(n > 1L) 

jobdata %>%
  filter(station_ID == 1, date == "2019-09-07", time == "03", indicator == "PM10")
...
piw_tab <- jobdata %>% 
  pivot_wider(
    id_cols = c(station_ID, coodrinates, date, time),
...
jobdata_v2 <- jobdata[!((jobdata$indicator == "Temperature" & jobdata$value < -34.1) | (jobdata_v2$indicator == "Temperature" & jobdata_v2$value > 37)),]
...
ggplot(
  ...
  geom_point(aes(color = coodrinates)) +
