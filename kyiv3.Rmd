...
ggplot(filtered_data, aes(x=indicator_CO2, y=indicator_CO)) + 
  ...
  labs(title="Залежність температури від вологості", x="Вологість", y="Температура")
```
...
```{r}
# Графік вологості
p1 <- ggplot(avg_data, aes(x=date, y=avg_humidity)) + 
  geom_line(color="blue") +
  labs(y="Вологість") +
  theme(axis.title.y=element_text(color="blue"))

# Графік температури
p2 <- ggplot(avg_data, aes(x=date, y=avg_temperature)) + 
  geom_line(color="red") +
  labs(y="Температура") +
...
# Графік PM10
p3 <- ggplot(avg_data, aes(x=date, y=avg_pm10)) + 
